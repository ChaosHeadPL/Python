...
def build_steam_url(appid, key, steamid):
    return f"http://api.steampowered.com/ISteamUserStats/GetPlayerAchievements" \
...
def combine_data(steam, eu4, map_achiev):
    ...
    Found errors:
        ...
        'You Get a New Home, and You Get a New Home' instead of 'You Get A New Home, and You Get a New Home'
        'I'll graze my horse here.. And here...' instead of 'I’ll graze my horse here.. And here…'
...
def main():
    ...
    # get steam data:
    response = get_request_response(url=build_steam_url(appid=steam_appid, key=steam_key, steamid=steam_id))
    steam_achievements = json.loads(response.decode('utf-8'))['playerstats']

    # get map steam_achievement -> eu4_achievement
    map_achievements = get_request_response(f"https://steamcommunity.com/profiles/{steam_id}/stats/{steam_appid}/?xml=1")
